...
pub extern "C" fn la_analyze(
    ...
    let samples_str    = match cstr_to_str(samples_json)    { Some(s) => s, None => return error("null samples_json") };
    let assertions_str = match cstr_to_str(assertions_json) { Some(s) => s, None => return error("null assertions_json") };
